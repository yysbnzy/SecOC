...
class SecOCAttacks:
    ...
    def replay_attack(self, msg_id: int, delay: float = 0.2) -> AttackResult:
        ...
        # Expected: Receiver should reject because MessageCounter has advanced
        return AttackResult(
            attack_name='Replay Attack',
            success=replayed,  # If replayed=True but should be rejected, attack succeeded
            details={
                'old_freshness': old_frame['freshness'],
                'old_cmac': f"0x{old_frame['cmac']:07X}",
                'replay_delay': delay,
    ...
    def freshness_rollback(self, msg_id: int, 
        ...
        # Validate locally
        local_valid = self.freshness.validate_freshness(
            old_trip, old_reset, current_fresh['message'], msg_id
        )
        
        return AttackResult(
            attack_name='Freshness Rollback',
            success=sent and not local_valid,  # Attack succeeds if sent but rejected
            details={
                'old_trip': old_trip,
                'old_reset': old_reset,
